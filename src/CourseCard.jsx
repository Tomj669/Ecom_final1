...
export default function CourseCard({ data }) {
  ...
  const navigate = useNavigate()
    return (
      ...
      <div  onClick={()=>{navigate('/course')}} className="aspect-video overflow-hidden rounded-t-lg">
        <img
      ...
      <div className="p-4 space-y-2">
        <h3 className="font-bold text-lg line-clamp-2 h-[50px]">{data.courseName}</h3>
        <p className="text-sm text-gray-600">{data.teacher}</p>
        <div className="flex items-center space-x-1">
          <span className="text-sm font-medium">{data.rating}</span>
          <ReviewStar data={data.rating} />
        </div>
            <div className="flex justify-between pr-2"> <p className="font-bold text-lg">${data.price.toFixed(2)}</p> 
